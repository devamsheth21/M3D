...
sys.path.append('/media/Datacenter_storage/Devam/CT/M3D')
# from Bench.dataset.otherdatasetv1 import CapDataset
from LaMed.src.dataset.ImagepartsCapdatasetv2 import CapDataset
...
from LaMed.src.model.language_model.lamed_phi3_v1 import LamedPhi3ForCausalLM
...
class ModelArguments:
    ...
    # aggregator
    num_heads: int = field(default=4, metadata={"help": "Number of attention heads in the Aggregator (Self-attention)."})
...
def main():
    ...
    # if you don't need segvol, set seg_enable to False
    model_args.segmentation_module = None
    model_args.pretrain_seg_module = None
    # if you don't need vision, set vision_tower to None
    model_args.pretrain_mm_mlp_adapter = None
    model_args.pretrain_vision_model = None
    ...
    # Attention Aggregator is trainable
    model.get_model().initialize_attention_aggregator(model_args= model_args)

    for param in model.get_model().attention_aggregator.parameters():
    ...
    if training_args.lora_enable:
        ...
        for n, p in model.named_parameters():
            if any(
                    [x in n for x in ['vision_tower', 'mm_projector', 'embed_tokens', 'lm_head', 'seg_projector', 'seg_module', 'attention_aggregator']] ## attention_aggregator
            ):
                p.requires_grad = True
    ...
    # if model_args.tune_mm_mlp_adapter:
        # full_dataset = TextDatasets(data_args, tokenizer, mode='train')
    # else:
        # full_dataset = UniDatasets(data_args, tokenizer, mode='train')
    full_dataset = CapDataset(data_args, tokenizer, mode='train')
